export class canvasLocal {
  private dibujador: CanvasRenderingContext2D;
  private medidaCelda: number;

  constructor(private superficie: HTMLCanvasElement) {
  ...
  private marcarCelda(ejeX: number, ejeY: number, relleno: boolean) {
    ...
    this.dibujador.fillRect(
      Math.floor(ejeX * this.medidaCelda),
      Math.floor(ejeY * this.medidaCelda),
      Math.ceil(this.medidaCelda),
      Math.ceil(this.medidaCelda)
    );
  }

  //* Método para establecer los patrones de alineación del QR
  private establecerPatrones() {
  ...
  private aplicarPatron(desplazX: number, desplazY: number, forma: number[][]) {
  ...
  private construirCodigo(info: string) {
    ...
    const bitsDatos = Array.from(info)
      .flatMap(c => 
        c.charCodeAt(0).toString(2).padStart(8, '0').split('')
      );
  ...
  private enZonaProtegida(x: number, y: number): boolean {
    ...
    const enPatron = patrones.some(p => 
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const contenedorQR = document.querySelector('qrinfo') as HTMLCanvasElement;
